...
//     TClonesArray* genParticleTCA = new TClonesArray("npknu::GenParticle")    ;         inChain->SetBranchAddress("genParticle"  , &genParticleTCA );
	...
	int nomenatorch =0;
     ...
     for(int eventLoop=0; eventLoop<TotalN ; eventLoop++) {
        ...
        bool isElePreselect = npknu::GetElectronPreselection(electronTCA, 35.0, 0.0, 1.4442 ); /* ptCut, etaCut*/
        ...
        if(isHLT115 ){ heb_pass1[0]->Fill( heepPtr->pt ,tWeight  );  heb_pass1[1]->Fill( heepPtr->pt ,tWeight  );  }
        if(isHLT115 || isPhotonpass){ heb_pass2[0]->Fill( heepPtr->pt ,tWeight  ); heb_pass2[1]->Fill( heepPtr->pt ,tWeight  ); }                            
	...
	denomenator++;
        ...
        if(isHLT115){
	    nomenator++;
            heb_pass1[2]->Fill( heepPtr->superCluster_eta  ,tWeight  );                 
            heb_pass1[3]->Fill( nVtx   ,tWeight );                                      
            heb_pass1[4]->Fill( heepPtr->full5x5_sigmaIetaIeta   ,tWeight );  
            heb_pass1[5]->Fill( heepPtr->hadronicOverEm    ,tWeight );  
            heb_pass1[6]->Fill( heepPtr->dEtaSeedAtVtx     ,tWeight );  
            heb_pass1[7]->Fill( heepPtr->deltaPhiSuperClusterTrackAtVtx   ,tWeight );  
	     ...
	     nomenatorch++;
             heb_pass2[2]->Fill( heepPtr->superCluster_eta  ,tWeight  );                 
             heb_pass2[3]->Fill( nVtx   ,tWeight );                                      
             heb_pass2[4]->Fill( heepPtr->full5x5_sigmaIetaIeta   ,tWeight );  
             heb_pass2[5]->Fill( heepPtr->hadronicOverEm    ,tWeight );  
             heb_pass2[6]->Fill( heepPtr->dEtaSeedAtVtx     ,tWeight );  
             heb_pass2[7]->Fill( heepPtr->deltaPhiSuperClusterTrackAtVtx   ,tWeight );  
        ...
        if(!isHLT115 &&  isPhotonpass){
             cout << "@@ EB Only PhoPass run:lumi:evt " << evtPtr->run << ":" << evtPtr->lumi << ":" << evtPtr->event << " Ele115 " <<  isHLT115 << " Pho175 " << isPhotonpass << "===>";
	     heepPtr->PrintObj();
         }
        if(!isHLT115 && !isPhotonpass ){
             cout << "## EB All 2HLTFail run:lumi:evt " << evtPtr->run << ":" << evtPtr->lumi << ":" << evtPtr->event << " Ele115 " <<  isHLT115 << " Pho175 " << isPhotonpass << "===>";
	     heepPtr->PrintObj();
         }
   ...
   heb_pass2[7]->Write() ; 
    ...
    outFile->Write();
     ...
     cout << " NunRepHitnomch   "<<   NunRepHitnomch   <<   endl;
        ...
        cout << " EB HLT+pho = " << nomenatorch << "   " <<int(heb_pass2[1]->Integral()) << endl;
        cout << "  # hlt/Tot = " <<  (double(nomenator)/double(denomenator)) << " %    " << ((double(heb_pass1[1]->Integral())/double(heb_total[1]->Integral()))*100) <<  " %" << endl;
        cout << "  # hlt+pho/Tot = " << (double(nomenatorch)/double(denomenator)) << " %    " <<  ((double(heb_pass2[1]->Integral())/double(heb_total[1]->Integral()))*100) <<  " %" << endl;
