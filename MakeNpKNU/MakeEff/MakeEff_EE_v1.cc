...
/*     TClonesArray* photonTCA      = new TClonesArray("npknu::Photon")         ;       inChain->SetBranchAddress("photon"       , &photonTCA      ); */
     ...
     for(int eventLoop=0; eventLoop<TotalN ; eventLoop++) {
        ...
        if(!isHLT115 && (isECALpass || isPhotonpass) ){
             cout << "@@ EE 115 Fail run:lumi:evt " << evtPtr->run << ":" << evtPtr->lumi << ":" << evtPtr->event << " ^HLT_Ele115 " <<  isHLT115 << " ^ECALPass " <<  isECALpass  << " ^PhotonPass " << isPhotonpass << endl;
	     heepPtr->PrintObj();
         }
        if(!isHLT115 && !isECALpass && !isPhotonpass){
             cout << "# EE All Fail run:lumi:evt " << evtPtr->run << ":" << evtPtr->lumi << ":" << evtPtr->event << " #HLT_Ele115 " <<  isHLT115 << " #ECALPass " <<  isECALpass  << " #PhotonPass " <<isPhotonpass << endl;
	     heepPtr->PrintObj();
         }
