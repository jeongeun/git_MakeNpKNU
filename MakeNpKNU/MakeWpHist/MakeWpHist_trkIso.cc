   ...
   bool isMuPreselection = false;
     ...
     for(int eventLoop=0; eventLoop<TotalN ; eventLoop++) {
       ...
       double highPtmuonPt1 = 0.0;
        ...
        cout << "Num " << muIdTCA->GetEntries() << " 1 highPtId Muon " << highmuPtr0->pt_TPq << " gb " << highmuPtr0->pt_GBq << " bt " << highmuPtr0->pt_BTq << " in " << highmuPtr0->pt_INq <<  ", eta " << highmuPtr0->eta << " phi " << highmuPtr0->phi << " trkSumPt " << highmuPtr0->TrkIsoR03_sumPt <<" relTrkIso " << highmuPtr0->relTrkIsoR03_BT <<  endl;

        if(muIdTCA->GetEntries() > 1){  
           npknu::Muon* highmuPtr1 = (npknu::Muon*)muIdTCA->At(1);
           highPtmuonPt1 = highmuPtr1->pt;
        cout << " 2 highPtId Muon " << highmuPtr1->pt_TPq << " gb " << highmuPtr1->pt_GBq << " bt " << highmuPtr1->pt_BTq << " in " << highmuPtr1->pt_INq <<  ", eta " << highmuPtr1->eta << " phi " << highmuPtr1->phi << " trkSumPt " << highmuPtr1->TrkIsoR03_sumPt << " relTrkIso " << highmuPtr1->relTrkIsoR03_BT <<  endl;
	...
	for(int i=0; i< muonTCA->GetEntries(); i++){
           ...
           if(muPtr->pt > highPtmuonPt0) {cout << "Muon pt " << i << " : "  << muPtr->pt_TPq << " gb " << muPtr->pt_GBq << " bt " << muPtr->pt_BTq << " in " << muPtr->pt_INq <<  ", eta " << muPtr->eta << " phi " << muPtr->phi << " trkSumPt " << muPtr->TrkIsoR03_sumPt <<" relTrkIso " << muPtr->relTrkIsoR03_BT << " ,ID Soft " << muPtr->isSoftMuonGV << ", Tight " <<  muPtr->isTightMuonGV << ", HighPt " << muPtr->isHighPtMuonGV <<  endl;}
 
	   if(muPtr->pt < 25.0)          
          if(muPtr->pt > 50 and muPtr->relTrkIsoR03_BT > 0.1) cout << "muon " << i << " was filtered by TrkIsoR03 = " << muPtr->relTrkIsoR03_BT << " > 0.1 " << endl;
        }       
        if(muonTCA->GetEntries()>1 ){
	       npknu::Muon* mu1Ptr = (npknu::Muon*)muonTCA->At(0);
	       npknu::Muon* mu2Ptr = (npknu::Muon*)muonTCA->At(1);
	       dR = mu1Ptr->deltaR(mu2Ptr); 
          bool indR03 = (dR < 0.3);
          cout << "Check deltaR = " << dR <<  " < 0.3 :" << indR03 <<endl;
       }

//              double dEta = (muonPtr->eta - genPtr->eta) ; //cout << dEta << endl;
//              double dPhi = (muonPtr->phi- genPtr->phi) ;  //cout << dPhi << endl;
//               dPhi = ( dPhi > TMath::Pi() ) ? TMath::TwoPi() - dPhi : dPhi;
//
//              double dR = TMath::Sqrt(dEta*dEta + dPhi*dPhi);
//              if(dR > 0.25) genMatch = false;

       cout << "Check HighPt Muon " << endl;
        /*****************************************************************************/
...
//        cout << "@@@ NumEvt METnoBadPF  = " << NnoBadPFmu << " / " << TotalN    << " = " << 100 - ((double(NnoBadPFmu)/double(TotalN ))*100) <<" %"  << endl ;
