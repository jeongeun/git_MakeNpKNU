...
/*     TClonesArray* photonTCA      = new TClonesArray("npknu::Photon")         ;       inChain->SetBranchAddress("photon"       , &photonTCA      ); */
     ...
     if(type == 1) {
        ...
        puWeight = new npknu::PUReweight("./etc/pileup/pu_RunG-H.root" , argv[5] , "MakeNpKNUHist/pileupHist/h1_TrueNumInteractions100");
     }
...
//   int Nkindphi  = 0;
   ...
   bool isMETBadGlobalMuon = false;
   bool isCorrectedGlobalMu = false;
   bool isMETBadDuplMu = false;
   bool isCorrectedDupliMu = false;

   int Nmupt100  = 0;
   int NBadGlobalMuon = 0;
   int NDuplicateMuon = 0;
   ...
   int NBadFirstHighPt =0;
     ...
     for(int eventLoop=0; eventLoop<TotalN ; eventLoop++) {
        ...
        isMETFilter = ( metPtr->Flag_HBHENoiseFilter == 1 &&  metPtr->Flag_HBHENoiseIsoFilter == 1 && metPtr->Flag_goodVertices ==1 && metPtr->Flag_CSCTightHaloFilter ==1 && metPtr->Flag_EcalDeadCellTriggerPrimitiveFilter ==1 && metPtr->Flag_eeBadScFilter ==1 && (metPtr->Flag_globalSuperTightHalo2016Filter ==1 ||  metPtr->Flag_globalTightHalo2016Filter ==1) ) ;
        ...
        //if(metPtr->IsBadGlobalMuonTagger == 1){NBadGlobalMuon++;}//Count BadGBMu

        isMETBadGlobalMuon = (metPtr->Flag_badGlobalMuonFilter  == 1) ;
        if(isMETBadGlobalMuon){NBadGlobalMuon++;}//Count BadGBMu
        isCorrectedGlobalMu = (  metPtr->IsCorrectedBadGlobalMuon == 1 ) ;//goodEvent
        if(isCorrectedGlobalMu ) NCorrectedGBMu ++;
        isMETBadDuplMu = (  metPtr->Flag_duplicateMuonFilter == 1 ) ;//goodEvent
        if(isMETBadDuplMu ) NDuplicateMuon ++;
       ...
       bool VetoPass = false;
        ...
        cout << "1st highPtId Muon " << highmuPtr0->pt_TPq << " gb " << highmuPtr0->pt_GBq << " bt " << highmuPtr0->pt_BTq << " in " << highmuPtr0->pt_INq <<  ", eta " << highmuPtr0->eta << " phi " << highmuPtr0->phi << " trkSumPt " << highmuPtr0->TrkIsoR03_sumPt <<" relTrkIso " << highmuPtr0->relTrkIsoR03_BT << " CorrBadGlobalMu " << isMETBadGlobalMuon << " CorrBadDupMu " << isMETBadDuplMu << endl;
        ...
        cout << " 2 highPtId Muon " << highmuPtr1->pt_TPq << " gb " << highmuPtr1->pt_GBq << " bt " << highmuPtr1->pt_BTq << " in " << highmuPtr1->pt_INq <<  ", eta " << highmuPtr1->eta << " phi " << highmuPtr1->phi << " trkSumPt " << highmuPtr1->TrkIsoR03_sumPt << " relTrkIso " << highmuPtr1->relTrkIsoR03_BT << " CorrBadGlobalMu " << isMETBadGlobalMuon << " CorrBadDupMu " << isMETBadDuplMu<<  " dR " << highmuPtr0->deltaR(highmuPtr1) << endl;
   ...
   cout << "Num of BadMet      = " << Nmetfil    << endl ;
   cout << "Num of BadPFMu     = " << NBadPFMu << endl ;
   cout << "Num of noBadch     = " << NnoBadch   << endl ;
   cout << "Num of CorrBadGbMu = " << NBadGlobalMuon << endl;
   cout << "Num of CorrGBMu    = " << NCorrectedGBMu << endl;
   cout << "Num of CorrDupliMu = " << NDuplicateMuon << endl;
   cout << "Num of CorrDuMu    = " << NCorrectedDuMu << endl;
   cout << "Num of passGoodMET = " << NpassMET   << endl ;
   cout << "Num of mupt100     = " << Nmupt100   << endl ;
   cout << "Num of trigg       = " << Ntrigg     << endl ;
   cout << "Num of HighPt1     = " << NHighPt         << endl ;
   cout << "Num of 1FailTrk    = " << NHighPtFailTrk         << endl ;
   cout << "Num of HighPt2     = " << NHighPt2         << endl ;
   cout << "Num of 2FailTrk    = " << NHighPt2FailTrk         << endl ;
   cout << "Num of noHighPt Big= " << NnoHighPtButBig         << endl ;
   cout << "Num of ClosedR     = " << NClosedR     << endl ;
   cout << "Num of noHighPt< 25= " << NnoHighPtnoVeto25         << endl ;
   cout << "Num of ClosedR2    = " << NClosedR2     << endl ;
   cout << "Num of Bad1stHighPt= " << NBadFirstHighPt     << endl ;
   cout << "Num of GoodMu      = " << NGoodMu     << endl ;
   cout << "Num of BadMu       = " << NBadMu     << endl ;
   cout << "Num of GoodMuTest  = " << NGoodMuTest     << endl ;
   cout << "Num of BadMuTest   = " << NBadMuTest     << endl ;
